...
`index-name`은 bbc-news로 입력 해 주면 된다. `host-url`은 이전 단계에서 생성 한 Pinecone index의 `HOST`값을 넣어 주면 된다. 
...
public class DataServiceImpl implements DataService {
    ...
    private static final int BATCH_SIZE = 5;        // 데이터 저장 시 한번에 처리 할 데이터 갯수
